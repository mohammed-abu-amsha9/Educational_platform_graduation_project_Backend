                                ...
                                <div
                                    ...
                                    @foreach ($permistions as $permistion)
                                        <label class="flex items-center justify-between">
                                            <span
                                                class="text-gray-600 dark:text-zinc-300">{{ $permistion->permistion_name }}</span>
                                            <input type="checkbox" name="permistions[]" value="{{ $permistion->id }}"
                    ...
                    @forelse ($roles as $role)
                        <div
                            ...
                            <div class="absolute top-4 left-4 flex items-center gap-2">
                                <button
                                    onclick="openEditRoleModal('{{ $role->id }}', '{{ $role->role_name }}', {{ json_encode($role->permistions->pluck('id')) }})"
                                ...
                                <form method="POST" class="m-0" action="{{ route('roles.destroy', $role->id) }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                            ...
                            <div class="flex items-start gap-4">
                                <div class="w-full">
                                    ...
                                    <div class="flex items-center gap-2 mt-1 text-xs text-gray-500">
                                        <span>{{ $role->permistions->implode('permistion_name', '، ') }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        @empty
                            <div class="col-span-2 text-center py-16 text-gray-400 text-sm">
                                <i class="fa-solid fa-user-tag text-3xl mb-3 block"></i>
                        ...
                        <div id="editRoleModal"
                            ...
                            <div
                                ...
                                <form method="POST" action="" class="flex flex-col flex-1 overflow-hidden m-0"
                                    id="editRoleForm">
                                    @csrf
                                    @method('PUT')
                                    ...
                                    <div class="p-6 space-y-5 overflow-y-auto flex-1 max-h-[calc(100vh-16rem)]">
                                        <div>
                                            <label
                                                class="block text-[11px] font-bold text-gray-500 dark:text-slate-400 mb-1.5">اسم
                                                الدور</label>
                                            <input type="text" id="modalRoleName" name="role_name"
                                        ...
                                        <div>
                                            ...
                                            <div>
                                                <div
                                                    class="border border-gray-100 dark:border-slate-800 rounded-xl p-3 max-h-40 overflow-y-auto space-y-2 text-xs custom-scroll bg-slate-50/50 dark:bg-slate-800/30">
                                                    @foreach ($permistions as $permistion)
                                                        <label class="flex items-center justify-between cursor-pointer py-0.5">
                                                            <span
                                                                class="text-gray-600 dark:text-zinc-300">{{ $permistion->permistion_name }}</span>
                                                            <input type="checkbox" name="permistions[]"
                                                                value="{{ $permistion->id }}"
                                                                class="permission-checkbox accent-blue-600" />
                                                        </label>
                                                    @endforeach
                                                </div>
                                            </div>
...
@section('scripts')
    ...
    <script>
        function openEditRoleModal(id, roleName, assignedPermissionIds) {
            // 1. تحديد عناصر الفورم والحقول
            const form = document.getElementById('editRoleForm');
            const inputRoleName = document.getElementById('modalRoleName');

            // 2. تحديث رابط التحديث للراوت الخاص بالأدوار (تأكد من مطابقة المسار عندك)
            form.action = `/admin/roles/${id}`;

            // 3. إدخال اسم الدور الحالي
            inputRoleName.value = roleName;

            // 4. عمل فحص وتحديد للـ Checkboxes بناءً على الصلاحيات القادمة من قاعدة البيانات
            const checkboxes = document.querySelectorAll('.permission-checkbox');
            checkboxes.forEach(checkbox => {
                // تحويل القيمة لرقم للتأكد من مطابقة النوع مع مصفوفة الـ IDs
                const permissionId = parseInt(checkbox.value);

                // إذا كان المعرف موجوداً في مصفوفة الصلاحيات الممنوحة للدور، قم بتفعيله

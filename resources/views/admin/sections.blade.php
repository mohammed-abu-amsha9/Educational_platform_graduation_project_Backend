            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                <div class="lg:col-span-4 order-1">
                    <div
                        ...
                        <form method="POST" action="{{ route('sections.store') }}" class="space-y-4">
                            @csrf
                            <div>
                                <label
                                ...
                                <input name="section_name" type="text" placeholder="مثال:أ"
                            ...
                            <div>
                                ...
                                <select name="grade_id"
                                    ...
                                    <option value="">اختر الصف الذي تريد ربطه في الشعبة...</option>
                    ...
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-lg font-black text-slate-800 dark:text-zinc-100 flex items-center gap-2">
                            ...
                            <span
                                ...
                                id="teacherCount">{{ $sections->count() }}</span>
                        </h2>
                    </div>

                    @forelse ($sections as $sectionGrade)
                        <div
                            ...
                            <div class="absolute top-4 left-4 flex items-center gap-2">
                                ...
                                <button
                                    onclick="openEditSectionModal('{{ $sectionGrade->id }}', '{{ $sectionGrade->name }}', '{{ $sectionGrade->grade_id }}')"
                                ...
                                <form method="POST" class="m-0"
                                    action="{{ route('sections.destroy', $sectionGrade->id) }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                            ...
                            <div class="flex items-start gap-4">
                                <div class="w-full">
                                    ...
                                    <div class="flex items-center gap-2 mt-1 text-xs text-gray-500">
                                        <span> {{ $sectionGrade->grade->name }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div
                                ...
                                <form method="POST" class="p-6 space-y-5 overflow-y-auto flex-1 max-h-[calc(100vh-16rem)]"
                                    ...
                                    <div>
                                        ...
                                        <input name="section_name" id="modalSectionName" type="text" placeholder="مثال:أ"
                                            class="w-full border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-800/50 rounded-xl py-2.5 px-4 text-sm outline-none focus:ring-2 focus:ring-teal-600 -all text-gray-400 dark:text-zinc-400" />
                                    </div>
                                    <div>
                                        <label class="block text-[11px] font-bold text-slate-700 dark:text-zinc-300 mb-1.5">الصف
...
@section('scripts')
    <script>
        function openEditSectionModal(id, sectionName, gradeId) {
            // 1. تحديد عناصر الفورم والمودال
            const form = document.getElementById('editSectionForm');
            const inputSectionName = document.getElementById('modalSectionName');
            const selectGrade = document.getElementById('modalGradeSelect');

            // 2. تحديث رابط الإرسال بالمعرف الخاص بالشعبة الحالية
            // تأكد من المسار الفعلي للراوت لديك (مثال: /admin/sections/ )

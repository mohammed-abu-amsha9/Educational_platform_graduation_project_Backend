<div wire:poll.2s
    ...
    <div class="relative flex min-h-[650px] lg:min-h-[700px]">
        ...
        <div id="teachersDrawer"
            ...
            class="absolute md:static inset-y-0 right-0 z-40 w-72 md:w-80 bg-white dark:bg-slate-900 border-l border-gray-200 dark:border-slate-800 flex-col shadow-xl md:shadow-none">

            <div class="p-4 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between bg-gray-50/50 dark:bg-slate-950/20">
            ...
            <div wire:ignore class="flex-1 overflow-y-auto divide-y divide-gray-100 dark:divide-slate-800/40">
                @foreach ($usersList as $user)
                    ...
                    <div wire:click="selectPerson({{ $user->id }})"
                        ...
                        <div class="w-8 h-8 rounded-xl bg-teal-600 text-white font-black flex items-center justify-center text-xs shrink-0">
        ...
        <div class="flex-1 flex flex-col bg-white dark:bg-slate-900 w-full">
            @if ($roomId && $recipient)
                <!-- Header المحادثة -->
                <div class="p-4 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between bg-gray-50/30 dark:bg-slate-950/10">
                    <div class="flex items-center gap-3">
                        <!-- زر فتح القائمة الجانبية (يظهر دائماً للتحكم بها) -->
                        <button @click="sidebarOpen = !sidebarOpen"
                            class="bg-gray-100 dark:bg-slate-800 text-slate-600 dark:text-zinc-300 p-2 rounded-xl hover:bg-teal-50 hover:text-teal-600 transition cursor-pointer"
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <div class="w-8 h-8 rounded-xl bg-teal-600 text-white font-black flex items-center justify-center text-xs">
                ...
                <div class="flex-1 p-5 overflow-y-auto space-y-4">
                    @foreach ($messages as $msg)
                        ...
                        <div class="flex items-start gap-2.5 {{ $isMe ? 'flex-row-reverse' : '' }}">
                            <div class="w-8 h-8 rounded-full bg-slate-700 text-white flex items-center justify-center text-xs">
                                {{ mb_substr($isMe ? 'أنا' : $recipient->full_name ?? 'م', 0, 1) }}
                            </div>

                            <div class="p-3 rounded-2xl shadow-sm text-sm {{ $isMe ? 'bg-teal-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-800 dark:text-zinc-100' }}">
                                {{ $msg->message_text }}
                            </div>
                        </div>
                    @endforeach
            ...
            @else
                ...
                <div class="flex-1 flex flex-col items-center justify-center text-gray-400 p-4">
                    <!-- زر إظهار القائمة حتى لو لم يتم اختيار محادثة -->
                    <button @click="sidebarOpen = !sidebarOpen"
                        class="mb-4 bg-teal-600 text-white px-4 py-2 rounded-xl flex items-center gap-2 cursor-pointer md:hidden">
                        <i class="fa-solid fa-bars"></i> اختر معلماً من القائمة
                    </button>
